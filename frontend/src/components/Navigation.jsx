            ...
            <div className="max-w-6xl mx-auto px-6 py-4">
                <div className="flex justify-between items-center">
                    ...
                    <div className="hidden md:flex space-x-20 ">
                        {navItems.map((item) => (
                            <a key={item.href} href={item.href} className={`transition-colors px-6 py-2 rounded-full duration-700 hover:bg-gray-100 ${isScrolled ? 'text-gray-600 hover:text-black ' : 'text-gray-700 hover:text-black'}`} onClick={(e) => {
                                e.preventDefault();
                ...
                <div className={`md:hidden transition-all duration-300 ease-in-out ${isMobileMenuOpen ? 'max-h-64 opacity-100 mt-4' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                    <div className="bg-white border border-gray-100 rounded-lg shadow-lg p-4 space-y-4 text-center">
                        {navItems.map((item) => (
